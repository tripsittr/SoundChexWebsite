...
- **W-26 — Open-source credits page.** `/docs/credits` credits all 822 third-party dependencies (PHP/npm/Rust) with name, version, licence and link, plus a curated "Built on" logo-tile marquee for the marquee frameworks. Data in resources/data/credits.json (from the app repo's manifests). Docs-only access (docs index + sidebar; not in nav/footer). *(changelog/014)*
